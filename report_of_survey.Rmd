---
title: "Survey of Study Area"
description: |
  Study Area
site: distill::distill_website
output:
  distill::distill_article:
      toc: TRUE
---
## Population and Demographics
![](Layout/population.jpg)
According to Map 1.1, almost all sub-districts with highest population quantile (13134 – 45505) are located in Balikpapan Regency and Samarinda Regency. Other two regencies have relatively lower population compared to these two. Hence, we can consider that Kota Balikpapan and Kota Samarinda has the biggest community settlement.  
![](Layout/dominantagegroup.jpg)
According to Map 1.2, the sub-districts with highest population are observed within Kota Balikpapan and Kota Samarinda. Majority of the sub-districts in Penajam Paser Utara are found to have the least share of population among four regencies. Sub-districts in Kutai Kartanegara have moderate to low population density. Hence, we concluded that the regencies of Penajam Paser Utara and Kutai Kartanegara will be fit for building a new national capital and a community settlement to redistribute the population more fairly across the island.  

![](Layout/populationdensity.jpg)
 
According to Map 1.3, majority of the sub-districts within our study-area have dominantly higher proportion of working adults. Hence, we can conclude that the local community has potential labour force that the new capital city development might need. 

## Economic and Businesses
![](Layout/sitedistribution.png)
In Indonesia, the economy is accounted with the Industrial, Mining and Agriculture. It was stated that the industrial sector account for up to about 40% of GDP in 2015 which includes natural gas, textiles, apparel, and significantly mining. Whereas, the Agriculture sector is accounted for about 14% 
![](Layout/Agriculturedistribution.png)
It represents the manufacturing/mining industrial sector found in the study area. From this we can distinctively identify that Balikpapan city have higher number of manufacturing/mining industrial sector in their economy as more of the site are observed here. 

With Indonesia earning the name of the world’s largest palm oil producer in the world, we can infer that Indonesia has the largest plantation area forming the concentration of agriculture's activities in the area. Food crops are cultivated in sawah (wetland),, Tegalan and ladang(dryland/garden). Sawah is a flat lowland on downhill sites or floodplains and terraced land on upper slopes. On the other hand, Tegalan  is a gentle slope land and usually has no access to any irrigation system and located close to settlement and ladang, a slope land of different variations of steepness and altitude. 


## Transport and Communication
To properly understand the Transport aspect of Kalimantan, we will leverage the following features for a wholistic analysis: 

Road Networks 

Bus Terminals 

Airports 

Sea Ports 

To begin we will start by analysing the road networks within the specified study area. The purple lines represent roads. Briefly, 2 major road clusters could be identified in Kota Balikpapan and Kota Samarinda. In Penajam Paser Utara, there seems to be a low road density. The overall road network in Kutai Kartaanergara seems to be substantial apart from the south region. 
![](Layout/roadnetwork.jpg)
Next, we will be analysing the density of roads within the study area. Kota Samarinda is densely connected with roads. It is by far the most connected district within our study area. Kota Balikpapan is the second most connected district with over half of its land well connected. The majority of  Kartaaneragra relates to roads, leaving only a small proportion not well connected. Penajam Paser Utara is the least accessible, it has a huge ration of land that is inaccessible by roads.

![](Layout/transport.jpg)
Shown above is a map depicting the distribution of transport infrastructure in our study area. The infrastructures include bust terminals, airports, and harbours. Kota Samarinda is the most accessible district within proximity to 3 bus terminals, multiple harbours, and an airport. Kota Kartaangara is the second most accessible district with 5 harbours, 2 bus stops and an airport.  

 
![](Layout/communication.jpg)
To survey the prominence of communication in our study area, we have plotted a map to depict the distribution of core communication infrastructures such as TV stations, Radio stations, phone towers and post offices. Kota Samarinda is the most well wired with communication infrastructure, with an abundance of phone towers and post offices despite its small land size. Kutai Kartaanegara is the second most well wired with a nice distribution of phone towers in the north and central region. Kutai Kartaanegara is also able to tap on the communication infrastructure of Kota Samarinda for higher usage. Kota Balikpapan is sufficiently wired with communication infrastructure for its size. Penajam Paser Utara is the least connected district with only 4 infrastructures at the extreme ends of its district parameters. 


## Infrastructure 
![](Layout/building.jpg)
From the above settlement choropleth map, it could be identified briefly that there are several distinct urban clusters. Kota Balikpapan is small but extremely dense. Kota Samarinda is also another small district with a dense settlement. Penajamm Paser Ultra on the other hand is significantly less dense than its neighbouring districts. Kutai Kartaanegara is not very dense overall, but contains several clusters of significant settlements. 


![](Layout/electricity.jpg)
Based on the topological dataset, there are 4 main points of electricity. Most of them are near and within the service range of Kota Samarinda. Therefore, Kota Samarinda has the highest accessibility to electricity. Although Kutai Kartaanegara has the greatest number of electricity points, its huge surface area is more underserved than Kota Samarinda. Kota Balikpapan might not have any points of electricity but it has access to the nearest electrical point in Kutai Kartaanegara. Penajam Paser Utara on the other hand does not have any electrical points and has the overall poorest access to electricity. 


## Environment and Hazard
![](Layout/naturalforest.jpg)
According to Map 1.6, the study area has dominant land-cover of natural forests, which we needed to avoid when implementing the new capital city. Majority of the forest areas are scrub, swamp scrubs and secondary swamp forests. A small proportion of dryland forest and mangrove forests were also observed.  


![](Layout/heatmap.jpg)
According to Indonesian National Board for Disaster Management (BNPB), East Kalimantan area is most prone to forest fires, compared to other hazards like volcanic eruptions, earthquakes, tsunamis, floods, and landslides. As seen in Map 1.7, huge portion of the study area falls under the high risk zone of the forest fire (according to data collected over 2014-2019). Hence, these areas should be avoided when selection for new capital is made.  


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

# Learn more about creating websites with Distill at:
# https://rstudio.github.io/distill/website.html

```





